...
if [ -e /sys/fs/pstore/console-ramoops ]; then
    cat /sys/fs/pstore/console-ramoops > $APLOG_DIR/lastkmsg
    Log "Collecting console-ramoops done"
fi
...
if [ -e /dev/block/bootdevice/by-name/logs ]; then
    date  >> $BL_LOGFILE
    echo "" >> $BL_LOGFILE
    cat /dev/block/bootdevice/by-name/logs >> $BL_LOGFILE
fi
...
for svc in qseelog batterylog mainlog mainlog_big radiolog radiolog_big kernellog eventslog eventslog_big; do
