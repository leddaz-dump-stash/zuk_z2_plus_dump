...
do
  ...
  if [ -n "$(getprop init.svc.qseelog)" ]; then
    if [ $(getprop init.svc.qseelog) = "running" ]; then
      echo "qseelog is running." >> $LOGFILE
      QSEE_LOG_STATUS=running
    else
      echo "qseelog is stopped" >> $LOGFILE
      QSEE_LOG_STATUS=stopped
    fi
  else
    echo "qseelog not run" >> $LOGFILE
    QSEE_LOG_STATUS=stopped
  fi
  ...
  if [ "$QSEE_LOG_STATUS" = "stopped" ]; then
    echo "qseelog services stopped, return shell right now." >> $LOGFILE
    return
  fi
...
stop qseelog
echo "qseelog stopped by timeout" >> $LOGFILE
stop kernellog
